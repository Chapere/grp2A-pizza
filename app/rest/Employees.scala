...
object Employees extends Controller {
  ...
  private case class HateoasEmployee(employee: Employee, url: String, name: String,
                                     lastname: String, workplace: String,
                                     acces: String, email: String, password: String)

  private def mkHateoasEmployee(employee: Employee)
                               (implicit request: RequestHeader): HateoasEmployee = {
  ...
  def employees: Action[AnyContent] = Action { implicit request =>
    ...
    Ok(Json.obj(
      employee -> Json.toJson(employees.map { employee =>
        Json.toJson(mkHateoasEmployee(employee))
      }),
      links -> Json.arr(
        Json.obj(
          rel -> self,
  ...
  private case class EmployeeName(name: String, lastname: String,
    ...
    employeename.fold(
      ...
      employeename => {
        Ok(Json.obj(status -> ok,
          employee -> Json.toJson(mkHateoasEmployee(EmployeeService.addEmployee(employeename.name,
            employeename.lastname, employeename.workplace,
            employeename.acces, employeename.accesLevel,
            employeename.netRate, employeename.email,
